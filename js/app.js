...
var CurrencyEX = React.createClass({
        ...
        render: function() {
            ...
            onClick = {
                this.handleAdd
            } > Add New Currency < /button> < div className = {
        ...
        } < /span> < i className = "fa fa-angle-down" > < /i > < /div> < div className = "dropdown-list" > < div > {
        this.renderListItems()
    } < /div> < /div > < span className = "datapeg" >
    < div className = {
        (this.state.selectedPair ? "view-display-clicked" : "view-display")
    } > {
        curData.UTCdate
    } < /div> < div className = { (this.state.selectedPair ? "view-display-clicked" : "view-display")
} > Rate: {
    this.state.rate
} < /div> < div className = { (this.state.selectedPair ? "view-display-clicked" : "view-display")
} > 1 {
    ...
    this.state.source
} < /div> < /span > < div className = "converter" >
    < p > Currency Converter < /p> < table > < tr > < td > < input id = "sourceCur"
type = "text"
value = {
...
< td > < span > {
    ...
    < td > < span id = "targetCur" > {
        this.state.targetCur
    } < /span></td >
    < td > < span > {
        this.state.target
    } < /span></td >
    < /tr> < tr > < td > < input id = "revSourceCur"
type = "text"
value = {
    this.state.curToRev
}
